    ...
    <div className={"w-full p-1 flex items-center gap-2" + (className ? " " + className : "")}>
      <svg className={"w-full h-10 overflow-visible"}>
        ...
        { marker && marker.map((d, i) => (
          <line key={i} x1={`${(d-axisMin) / axisRatio}%`} y1={1} x2={`${(d-axisMin) / axisRatio}%`} y2={13} strokeWidth={2} className={"stroke-blue-300"} strokeDasharray={"2 2"} />
        ))}
        { ticks.map((i: number, idx: number) => (
          <text key={`tick-${i}`} x={`${idx / 10 * 100}%`} y={40} className={"fill-gray-100"} textAnchor={"middle"}>{i}</text>
        ))}
      </svg>
    </div>
  );
